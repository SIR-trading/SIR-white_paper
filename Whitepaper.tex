...
%	Let $y$ be the the deposited/withdrawn collateral, and let $y_\textrm{fee}$ be a fee added on top, meaning the user needs a total of $y_\textrm{tot} = y+y_\textrm{fee}$.
	...
	\begin{enumerate}
		...
		\item Fees proportional to amount of liquidity used: The fees paid by gentlemen \eqref{eq:fee:tea} and apes \eqref{eq:fee:ape} are proportional to the amount of liquidity that they require. For instance, a user minting $y$ APE on a pool with target leverage $l_\textrm{tar}$ will require $(l_\textrm{tar}-1)yp$ TEA in liquidity in the pool. So it is logical that an ape pays more in fees when using $l_\textrm{tar}=9$ leverage than $l_\textrm{tar}=2$ leverage.
