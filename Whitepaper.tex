...
%	Let $y$ be the the deposited/withdrawn collateral, and let $y_\textrm{fee}$ be a fee added on top, meaning the user needs a total of $y_\textrm{tot} = y+y_\textrm{fee}$.
	...
	\begin{align}
		f_\textrm{tea}^\textrm{mint} &= \frac{0.003}{l_\textrm{tar}+l-2}  \label{eq:fee:tea_mint} \\
		f_\textrm{tea}^\textrm{burn} &= \frac{r-1}{\frac{r_\textrm{tar}-1}{r-1}+1} 0.003 \\
		f_\textrm{ape}^\textrm{mint} &= \frac{0.003}{r_\textrm{tar}+r-2} \\
	...
	\begin{equation}
		y_\textrm{fee} = 
		\begin{cases}
			\int_{0}^{y} f_\textrm{tea}^\textrm{mint} \,\mathrm{d}\varepsilon = 0.003 A \ln\left(\frac{l'+l_\textrm{tar}-2}{l+l_\textrm{tar}-2}\right)
			& \text{for minting TEA} \\
			\int_{0}^{y} f_\textrm{tea}^\textrm{burn} \,\mathrm{d}\varepsilon = 0.003 A \ln\left(\frac{r'+r_\textrm{tar}-2}{r+r_\textrm{tar}-2}\right)	
			& \text{for burning TEA} \\
			\int_{0}^{y} f_\textrm{ape}^\textrm{mint} \,\mathrm{d}\varepsilon = 0.003\frac{T}{p} \ln\left(\frac{r'+r_\textrm{tar}-2}{r+r_\textrm{tar}-2}\right)
			& \text{for minting APE} \\
			\int_{0}^{y} f_\textrm{ape}^\textrm{burn} \,\mathrm{d}\varepsilon = 0.003\frac{T}{p} \ln\left(\frac{l'+l_\textrm{tar}-2}{l+l_\textrm{tar}-2}\right)
			& \text{for burning APE}
		\end{cases}
	\end{equation}
